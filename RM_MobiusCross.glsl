...
float sdfCross(vec3 pos, vec3 box){
    return min(sdfBox(pos, box.xyz), sdfBox(pos, box.yzx));
}
...
float Mobius(vec3 q){
    ...
    p.xz *= r2(a);
    p.x -= toroidRadius;
    p.xy *= r2(a*polRot + u_time);  // Twisting about the poloidal direction (controlled by "polRot) as we sweep.
    ...
    float rail = sdfSphere(p, 0.27); // Makeshift octagon.
    
  	float ia = (floor(ringNum*a/6.2831853) + .5)/ringNum*6.2831853; 
    p = q;
    p.xz *= r2(ia); // Using the quantized angle to obtain the position of the center of the corresponding cell.
    p.x -= toroidRadius;
    p.xy *= r2(radians(45.) + a*polRot + u_time);  // Twisting about the poloidal direction - as we did with the rails.
    p = abs(p);
    float ring = sdfCross(p, vec3(0.02,0.3,0.02));
