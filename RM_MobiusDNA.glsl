...
float Mobius(vec3 p){
    ...
    float polRot = floor(6.)/4.; // Poloidal rotations.
    ...
    p.zy = abs(abs(p.zy) - .10); // Change this to "p = abs(p)," and you'll see what it does.

    rail = sdfSphere(p, vec3(0), 0.11);
